...
@register_env("MultiRobotPassStickDelta-v1", max_episode_steps=100)
class MultiRobotPassStickDelta(BaseEnv):
    """
    **Task Description:**
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        with torch.device(self.device):
            ...
            self._last_d_goal = torch.linalg.norm(stick_pos - goal_pos, axis=1)
            self._last_d_arms = torch.stack([
    ...
    def compute_dense_reward(self, obs: Any, action: torch.Tensor, info: dict):
        ...
        delta_goal = self._last_d_goal - d_goal
        self._last_d_goal = d_goal.detach().clone()

        arms = self.agent.agents
        delta_arms = 0.0
